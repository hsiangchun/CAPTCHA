...
def gen_data(input_base_dir, image_list_filename, output_filebase, 
             num_shards=300,start_shard=0,exist=0):
    ...
    image_filenames = get_image_filenames(os.path.join(input_base_dir,
                                                       image_list_filename))
    num_digits = math.ceil( math.log10( num_shards - 1 )) #3
    ...
    for i in range(start_shard,num_shards):
        ...
        out_filename = output_filebase+'-'+(shard_format % (i+exist))+'.tfrecord' #words-000.tfrecord
        if os.path.isfile(out_filename): # Don't recreate data if restarting
            print out_filename," is exist"
            continue
    ...
    # image_filenames[start:] will be NULL so it would not entry gen_shard()
    start = num_shards*images_per_shard
    out_filename = output_filebase+'-'+(shard_format % (num_shards+exist))+'.tfrecord'
...
def is_writable(image_width,text):
    ...
    # To prevent image too narrow and To prevent label of image too long to no space
...
def get_text_and_labels(filename):
    ...
    text = os.path.basename(filename).split('.',2)[0]
...
def main(argv=None):
    ...
    gen_data('../data/new/10', 'new.txt', '../data/train/words',num_shards=49706,exist=100000)     
